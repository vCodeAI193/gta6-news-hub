...
## Wave 14 — Performance, PWA & Offline (`src/lib/performance.ts`)

### Decisions
- **Blur-up placeholders** use inline SVG data URIs with a solid color.
  Fast to generate, zero network requests, satisfies the layout-shift
  requirement without adding an image processing pipeline.
- **Offline action queue** is localStorage-based. `queueOfflineAction` stores
  pending actions; callers can drain the queue on reconnect via
  `getOfflineQueue` + `clearOfflineAction`. No service worker background sync
  is wired (it requires a custom SW handler beyond what `vite-plugin-pwa`
  generates by default).
- **Data saver detection** uses `navigator.connection.saveData` and
  `effectiveType`. Only available in Chromium; falls back to `false` silently.
- **Vitals budget thresholds** are the Web Vitals "good" cutoffs per Google.
  `checkVitalsBudget` is a utility — it does not automatically gate the build.
  A real Lighthouse CI gate requires a separate `lighthouse-ci` action job.

### Critique
- **SSR, SSG, edge rendering, ISR** are architectural changes that would
  require migrating from Vite to Next.js or similar. They are documented stubs.
- **Background Sync API** is used via the offline queue concept, but actual
  SW background sync registration is not wired.
- **HTTP/3, Brotli, Critical CSS, Font Subsetting** are infrastructure
  concerns (CDN/server config), not frontend code. Documented as stubs.

### Improvement suggestions
- Register a `sync` event in `sw.js` that reads `gta6hub_offline_queue`
  and replays actions to the API, then clears them.
- Add a `lighthouserc.json` to the repo and a GitHub Actions job that runs
  `lhci autorun` with budget assertions for LCP/FID/CLS.
- Extend `imgSrcset` to generate WebP URLs when the image host supports
  on-the-fly conversion.

---

## Wave 15 — SEO, Growth & Marketing (`src/lib/seo.ts`, `/presse`)

### Decisions
- **OG image URL** points to `/api/og-image?title=...`. This endpoint is a
  stub (not wired in the backend); a real implementation would use `satori`
  or `sharp` to render an SVG/PNG server-side.
- **Structured data helpers** (`breadcrumbSchema`, `faqSchema`, `videoSchema`)
  produce valid JSON-LD strings for direct injection via `<script type="application/ld+json">`.
- **Auto internal linking** uses word-overlap heuristics. Links are surfaced
  as suggestions to the CMS editor; they are not injected into rendered HTML
  automatically (that would require a markdown AST transform).
- **Hreflang links** are generated client-side — useful for populating
  `<Helmet>` links, not for true server-side hreflang which requires all
  locale variants to exist at their respective URLs.
- **Google News sitemap** format is implemented as a data transformer;
  the actual sitemap file is generated by `scripts/generate-sitemap.mjs`
  which would need updating to emit the News namespace.

### Critique
- **OG image generation is a stub.** Without a running `/api/og-image`
  endpoint, social crawlers see no custom card image.
- **IndexNow/Ping auto-submission** requires an API key and a network call
  on publish — this is wired in `integrations.ts` as a webhook stub.
- **SEO audit** runs in the browser against localStorage articles. A real
  audit would scan the live rendered HTML and check for canonical tags,
  duplicate content, etc.

### Improvement suggestions
- Implement `/api/og-image` using `satori` (Vercel's SVG-from-JSX renderer)
  with a GTA 6 Hub branded template.
- Add `indexNowPing(url)` to `integrations.ts` using the IndexNow API with
  the key stored in `INDEXNOW_KEY` env var.
- Move the SEO audit to a Playwright script that crawls all article pages
  and generates a JSON report.

---

## Wave 16 — Analytics & Business Intelligence (`src/lib/analytics.ts` extended, `/analytics`)

### Decisions
- **Extended the existing `analytics.ts`** with event tracking, scroll depth,
  NPS survey, and UTM capture rather than creating a new file, keeping a
  single consistent module.
- **Event log is capped at 1000 entries** (the `slice(-999)` before append)
  to prevent unbounded localStorage growth.
- **NPS is purely client-side.** Responses persist in localStorage; average
  is computed locally. A real NPS system would aggregate server-side.
- **UTM capture** reads `window.location.search` on demand; it must be called
  early in the app lifecycle (e.g. in `main.tsx` or `useEffect` on mount) to
  capture UTM params before the user navigates away.
- **Funnel tracking** is implemented in the analytics module but no UI
  dashboard for funnels was added to keep scope manageable.

### Critique
- **All analytics are local.** The dashboard shows only the current device's
  events. Real BI needs server-side aggregation.
- **Heatmaps, session replay, cohort analysis** require third-party SDKs
  (Hotjar, PostHog) or significant custom infrastructure. They are stubs.
- **Scroll depth** is per-device; devices do not share depth data.

### Improvement suggestions
- Flush `analytics_events` to `/api/analytics/events` in batches (on page
  unload via `sendBeacon`) to build a real server-side event store.
- Add a `POST /api/nps` endpoint to aggregate NPS responses across all users
